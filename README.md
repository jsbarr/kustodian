...
A `join` differs from a `lookup`: it keeps **both** `on` keys as separate output columns (e.g. `DeviceId` and `DeviceId1`), so there is no single node to mark and the returned tree stays faithful — each key traces only to its own side, with no synthetic equality edge. The check still knows the keys are equal, though, and may attribute one key's leaf to the matched partner's record. This lets a detection that pivots through a join key — `… | join (Enrichment) on DeviceId | project Timestamp = OtherTimestamp, ReportId = OtherReportId, DeviceId` — pass, since `DeviceId` describes the same record as the enrichment columns. The direction follows the kind: `inner`/`innerunique` equate both sides (either key may borrow the other); a `leftouter`/`rightouter` guarantees only the surviving side, so the non-surviving key may borrow the surviving leaf but not the reverse; `fullouter`/semi/anti prove no cross-row equality and borrow nothing. (A same-table self-join collapses both keys to one leaf, so there is nothing to borrow — that case is unchanged.)
